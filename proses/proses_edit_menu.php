...
if (!empty($_POST['input_menu_validate'])) {
    // Mengcek apakah gambar atau bukan
    $cek = getimagesize($_FILES['foto']['tmp_name']);
    if ($cek === false) {
        $message = "Ini buka file gambar";
        $statusUpload = 0;
    } else {
        $statusUpload = 1;
        if (file_exists($target_file)) {
            ...
            if ($_FILES['foto']['size'] > 500000) { //500kb
                ...
                if ($imageType != "jpg" && $imageType != "png" && $imageType != "jpeg" && $imageType != "gif") {
                    $message = "Maaf, file bisa diupload yang hanya memiliki format JPG, PNG, JPEG dan GIF";
                    $statusUpload = 0;
                }
            }
        }
    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            $message = '<script>alert("Nama menu telah tersedia")
                    window.location="../menu"</script>';
        } else {
